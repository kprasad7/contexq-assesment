...
import pandas as pd
from data_contracts import MLFeatureContract, PreparedDataContract
...
def validate_etl_transformation():
    ...
    for idx, row in df.iterrows():
        ...
        if abs(expected_margin - actual_margin) < 0.01:  # 1% tolerance
            ...
            print(
                f"       Record {idx}: Margin mismatch ({actual_margin:.2%} vs {expected_margin:.2%})"
            )
            return False
...
def validate_ml_training_transformation():
    ...
    print(
        f"      - Class 0 (Low Profit): {label_dist.get(0, 0)} records ({label_dist.get(0, 0)/len(df)*100:.1f}%)"
    )
    print(
        f"      - Class 1 (High Profit): {label_dist.get(1, 0)} records ({label_dist.get(1, 0)/len(df)*100:.1f}%)"
    )
